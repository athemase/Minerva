...
public class Java_Scope implements Execution_Behavior{

	public int exe(DebugInfo info) throws IOException {
		int mode = DebugBridge.DO_NOTHING;
		MinervaServer.getInstance().sendMessage(JavaMsgFormat.formScopeResult
				(JavaMsgFormat.isVarOutOfScope(info.getMessage()),info.getVarName()));
		return mode;
	}
